(function () {
    ...
    const ASSET_ID_BYTE_LENGTH = 32;

    function AssetPicker($scope, $element, autocomplete, apiService, utilityService) {
        const ctrl = this;
        const autocompleteElement = $element.find(`md-autocomplete`);
        ...
        ctrl.$onChanges = function () {
            if (ctrl.assets && ctrl.pickedAsset) {
                ...
                ctrl.autocomplete.assets = ctrl.assets
                    .map((asset) => asset.currency)
                    .filter((asset) => asset !== ctrl.hiddenAsset)
                    .sort((a, b) =>
        ...
        autocompleteElement.on(`focusin`, () => {
            ctrl.isPickingInProgress = true;
        });

        autocompleteElement.on(`focusout`, () => {
            ctrl.isPickingInProgress = false;
            ctrl.autocomplete.selectedAsset = ctrl.pickedAsset;
        });

        ctrl.changeAsset = function () {
            const asset = ctrl.autocomplete.selectedAsset;
        ...
        ctrl.findAssets = function (query) {
            const assets = ctrl.autocomplete.querySearch(query);
            if (assets.length === 0 && isValidAssetId(query)) {
                ctrl.isAssetLoading = true;
                apiService.transactions.info(query)
                    .then((response) => {
    ...
    AssetPicker.$inject = [`$scope`, `$element`, `autocomplete.assets`, `apiService`, `utilityService`];

    angular
        .module(`app.dex`)
        .component(`wavesDexAssetPicker`, {
            controller: AssetPicker,
            bindings: {
                name: `@`,
                type: `@`,
                assets: `<`,
