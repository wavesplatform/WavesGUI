describe(`Transaction.Loading.Service`, () => {
    'use strict';
    ...
    it(`should merge transactions if confirmed and unconfirmed do not intersect`, () => {
        const confirmed = [
            {
                type: 2,
                fee: 59291,
        ...
        const tx = transactionService.mergeTransactions(account, unconfirmed, confirmed);

        expect(tx.length).toEqual(5);
        _.each(confirmed, (transaction) => {
            expect(_.indexOf(tx, transaction)).toBeGreaterThan(-1);
        });
        _.each(unconfirmed, (transaction) => {
            expect(_.indexOf(tx, transaction)).toBeGreaterThan(-1);
        });
    });

    it(`should merge transactions if confirmed and unconfirmed set intersect`, () => {
        const confirmed = [
            {
                type: 2,
                fee: 59291,
    ...
    it(`should filter unconnfirmed transactions and then merge them`, () => {
        const confirmed = [
            {
                type: 2,
                fee: 59291,
