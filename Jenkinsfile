                            ...
                            dir(Constants.DOCKERFILE_LOCATION_MAP[serviceName]) {
                                stage("Building " + serviceName) {
                                    ...
                                    if (pipeline_tasks['build']) {
                                        ...
                                        // run build
                                        ut.buildDockerImage('waves/' + serviceName, source.split("\\.")[0] + '.latest', "--build-arg trading_view_token=${Constants.WAVES_WALLET_TRADING_VIEW_TOKEN} --build-arg platform=${platform} --no-cache")
